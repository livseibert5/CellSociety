...
* Change the title for the segregation simulation from Segregation to Schelling’s Model of Segregation
    * Change the Title tag of the XML file of the game to be Schelling’s Model of Segregation. When the XML file is reloaded, the XML parser will take the Title tag and its value and put them into a HashMap with the other attributes of the game. This HashMap will then be passed to the controller for the game so that the new attributes can be used in this simulation. Since the controller has access to the new title, the cellsociety.visuals.Graphics class will then be able to display it in the game.

...
public class GameLoop extends Application {
  ...
  /**
   * continuously runs when the project is running. Starts the simulation. updates grid every time
   * it runs.
   *
   * @param elapsedTime
   * @throws IOException
  ...
  /**
   * returns the landing scene with the buttons of each simulation. Also has the option to change
   * language and background color.
   *
  ...
  public Scene creatingLandingScreen() {
    ...
    Text welcome = Graphics
        .constructText(baseY, 30, "Simulation Menu", FontWeight.BOLD, Graphics.FONT);

    //create a button for each simulation, set controller type and resourceBundle based on what simulation is chosen.
    //Game of Life
    Graphics.createButton(Graphics.myLandingSceneResources.getString("GameOfLifeSimulation"), 0,
        buttonsInVertical,
        event -> {
          try {
    ...
    //Wa-Tor
    Graphics.createButton(Graphics.myLandingSceneResources.getString("WaTorSimulation"), 30,
        buttonsInVertical,
        event -> {
          try {
    ...
    //Fire
    Graphics.createButton(Graphics.myLandingSceneResources.getString("FireSimulation"), 40,
        buttonsInVertical,
        event -> {
          try {
    ...
    //Ant Forager
    Graphics.createButton(Graphics.myLandingSceneResources.getString("AntSimulation"), 50,
        buttonsInVertical,
        event -> {
          try {
    ...
    Graphics
        .createButton(Graphics.myLandingSceneResources.getString("Custom"), 70, buttonsInVertical,
            event -> {
              try {
   ...
   *
   * @param root need to put it in as a parameter because it is used in the getComboBox method to
   *             add the ComboBoxes
   * @return a VBOX with the buttons that are needed in the landing screen to custom color and
   * language.
   */
  ...
  private Grid setTwoGrid(String filename, String secondFileName, Controller controllerType,
  ...
  private void updateGrid(ResourceBundle resourceBundle, Controller controller,
    ...
    if (secondSimulationStarted) {
      ...
      scene = secondGraphicsController
          .setGridView(grid, grid2, currentResourceBundle, event, simulationStarted,
              secondSimulationStarted);
    } else {
  ...
  private void setSpecificConfigFile(String fileName, Controller currentControllerType,
  ...
  private void openFileChooser(Controller currentControllerType,
      ResourceBundle currentResourceBundle)
      throws IOException, SAXException, ParserConfigurationException {
  ...
  private void openSecondFileChooser(String firstFileName, Controller secondController,
      ResourceBundle currentResourceBundle)
      throws IOException, SAXException, ParserConfigurationException {
    ...
    if (selectedFile != null) {
      secondSimulationStarted = true;
      String fileName = selectedFile.getName();
      setTwoGrid(firstFileName, fileName, currentControllerType, secondController,
  ...
  private Grid setCustomGrid(XMLParser parse, Controller controllerType,
      ResourceBundle simulationType)
      throws IOException, SAXException, ParserConfigurationException {
    simulationData = parse.getInfo();
    Grid grid = parse.getGrid();
  ...
  private ComboBox getComboBox(String[] listOfOptions, VBox root, int listNumber,
      double xPosition) {
  ...
  private void createSecondLandingScreen(Controller currentControllerType,
      ResourceBundle currentResourceBundle)
      throws IOException, SAXException, ParserConfigurationException {
